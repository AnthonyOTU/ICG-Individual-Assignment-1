...
void Application::changeLUT() {
	...
	if (glfwGetKey(_window, GLFW_KEY_8) == GLFW_PRESS) {
		Texture3D::Sptr lut = ResourceManager::CreateAsset<Texture3D>("luts/coolBlue.CUBE");
		CurrentScene()->SetColorLUT(lut);
	}

	if (glfwGetKey(_window, GLFW_KEY_9) == GLFW_PRESS) {
		Texture3D::Sptr lut = ResourceManager::CreateAsset<Texture3D>("luts/warmRed.CUBE");
		CurrentScene()->SetColorLUT(lut);
	}

	if (glfwGetKey(_window, GLFW_KEY_0) == GLFW_PRESS) {
		Texture3D::Sptr lut = ResourceManager::CreateAsset<Texture3D>("luts/neon.CUBE");
		CurrentScene()->SetColorLUT(lut);
	}
